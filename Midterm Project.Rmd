...
```
From the figure above I found that customers has not much difference in choosing each options. Also most of customers need more than five quotes then finally purchase.
...
# 4. Model Analysis
##4.1 Multilevel generalized linear model.
...
## 4.2 Purchase prediction with previous c option
```{r}
...
predict.1 <- glmer(record_type ~ C_previous + duration_previous  +(1 | plan), data=test, family = binomial, control = glmerControl(optimizer = "bobyqa"),nAGQ = 10)
summary(predict.1)
y_hat.3 <- fitted.values(predict.1)
res.3 <- resid(predict.1, type = "response")
binnedplot(y_hat.3,res.3)
```
